...
int main(int argc, char* argv[]){
  ...
  const unsigned DL1S = 500;
  const unsigned DL2S = 600;

/*
  // bisection
  NN::type td_dw = 1.0;    // tail_decay down (backtesting level is below 95)
  NN::type b95_dw = 0.0;   // backtest 95% down (initialization)

  NN::type td_up = 0.5;    // tail_decay up (backtesting level is above 95)
  NN::type b95_up = 0.0;   // backtest 95% up (initialization)

  NN::type td_md = (td_dw + td_up) / 2;;
  NN::type b95_md = 0.0;

  b95_up += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles1",  SS, 200, 300, td_up);
  b95_up += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles2",  SS, 300, 400, td_up);
  b95_up += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles3",  SS, 400, 500, td_up);
  b95_up /= 3;
  if (rank == 0) std::cout << td_up << " " << b95_up << std::endl;

  b95_dw += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles4",  SS, 200, 300, td_dw);
  b95_dw += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles5",  SS, 300, 400, td_dw);
  b95_dw += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles6",  SS, 400, 500, td_dw);
  b95_dw /= 3;
  if (rank == 0) std::cout << td_dw << " " << b95_dw << std::endl;

  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles7",  SS, 200, 300, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles8",  SS, 300, 400, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles9",  SS, 400, 500, td_md);
  b95_md /= 3;
  if (rank == 0) std::cout << td_md << " " << b95_md << std::endl;

  if (b95_md > 0.95){ td_up = td_md; b95_up = b95_md;}
  else{ td_dw = td_md; b95_dw = b95_md;}
  td_md = (td_dw + td_up) / 2;
  b95_md = 0.0;
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles10", SS, 200, 300, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles11", SS, 300, 400, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles12", SS, 400, 500, td_md);
  b95_md /= 3;
  if (rank == 0) std::cout << td_md << " " << b95_md << std::endl;
  ...
  td_md = (td_dw + td_up) / 2;
  b95_md = 0.0;
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles13", SS, 200, 300, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles14", SS, 300, 400, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles15", SS, 400, 500, td_md);
  b95_md /= 3;
  if (rank == 0) std::cout << td_md << " " << b95_md << std::endl;
  ...
  td_md = (td_dw + td_up) / 2;
  b95_md = 0.0;
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles16", SS, 200, 300, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles17", SS, 300, 400, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles18", SS, 400, 500, td_md);
  b95_md /= 3;
  if (rank == 0) std::cout << td_md << " " << b95_md << std::endl;
  ...
  td_md = (td_dw + td_up) / 2;
  b95_md = 0.0;
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles19", SS, 200, 300, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles20", SS, 300, 400, td_md);
  b95_md += main_aux(rank, size, 2,  15, 0.001, 32, {1}, "MPI_G/OutFiles21", SS, 400, 500, td_md);
  b95_md /= 3;
  if (rank == 0) std::cout << td_md << " " << b95_md << std::endl;


  const NN::type TD = 0.80;    // tail_decay

  main_aux(rank, size, 2,  5, 0.0001, 32, {1}, "MPI_G/OutFiles1",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10, 0.0001, 32, {1}, "MPI_G/OutFiles2",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15, 0.0001, 32, {1}, "MPI_G/OutFiles3",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2,  5, 0.0005, 32, {1}, "MPI_G/OutFiles4",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10, 0.0005, 32, {1}, "MPI_G/OutFiles5",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15, 0.0005, 32, {1}, "MPI_G/OutFiles6",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2,  5,  0.001, 32, {1}, "MPI_G/OutFiles7",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10,  0.001, 32, {1}, "MPI_G/OutFiles8",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15,  0.001, 32, {1}, "MPI_G/OutFiles9",  SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2,  5, 0.0001, 64, {1}, "MPI_G/OutFiles10", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10, 0.0001, 64, {1}, "MPI_G/OutFiles11", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15, 0.0001, 64, {1}, "MPI_G/OutFiles12", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2,  5, 0.0005, 64, {1}, "MPI_G/OutFiles13", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10, 0.0005, 64, {1}, "MPI_G/OutFiles14", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15, 0.0005, 64, {1}, "MPI_G/OutFiles15", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2,  5,  0.001, 64, {1}, "MPI_G/OutFiles16", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 10,  0.001, 64, {1}, "MPI_G/OutFiles17", SS, DL1S, DL2S, TD);
  main_aux(rank, size, 2, 15,  0.001, 64, {1}, "MPI_G/OutFiles18", SS, DL1S, DL2S, TD);
*/

  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles1",  SS, 100,  200,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles2",  SS, 200,  300,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles3",  SS, 300,  400,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles4",  SS, 400,  500,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles5",  SS, 500,  600,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles6",  SS, 600,  700,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles7",  SS, 700,  800,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles8",  SS, 800,  900,  1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles9",  SS, 900,  1000, 1.00);
  main_aux(rank, size, 2, 15,  0.001, 32, {1,2,24}, "MPI_G/OutFiles10", SS, 1000, 1100, 1.00);
...
NN::type main_aux(int rank, int size,
  ...
  NN::type learning_rate, unsigned batch_size,
  std::vector<unsigned> which_lags, std::string foldername_outfiles,
  unsigned shuffling_seed, unsigned dl1_seed, unsigned dl2_seed, NN::type tail_decay){
    ...
    //std::ignore = dt_train.reduce_dataset(">=", "2010-01-03 00:00:00");
    //std::ignore = dt_train.reduce_dataset("<",  "2011-01-01 00:00:00");
    //std::ignore = dt_train.reduce_dataset(">=", "2011-01-01 00:00:00");
    std::ignore = dt_train.reduce_dataset(">=", "2010-01-01 00:00:00");
    std::ignore = dt_train.reduce_dataset("<",  "2014-01-01 00:00:00");

    Dataset dt_test(filename_);
    //std::ignore = dt_test.reduce_dataset(">=", "2011-01-01 00:00:00");  //should be 2010-12-30!
    //std::ignore = dt_test.reduce_dataset("<",  "2012-01-01 00:00:00");
    std::ignore = dt_test.reduce_dataset(">=", "2013-12-30 00:00:00");
    std::ignore = dt_test.reduce_dataset("<",  "2015-01-01 00:00:00");
    ...
    const unsigned DL1_SEED = dl1_seed;
    const unsigned DL2_SEED = dl2_seed;
    const unsigned SHUFFLING_SEED = shuffling_seed; //40 //20; // original 10
    ...
    if (NN::OUTPUT_NODES == 1){
        ...
        //rnn.set_loss("GaussianCRPS"); 		// integral of APL, original (lambda=1)
    ...
    // initialize where to store backtesting 95%
    NN::type backtest95 = 0.0;
    ...
    if (rank==0) {
        ...
        output_log << "\n" << "Tail Decay: " << tail_decay << std::endl;
        ...
        MPI_Recv(&backtest95, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
