...
void RnnModel::fit(const std::vector< std::vector<VectorClass> >& x_sequences_train,
    ...
    for (unsigned epoch_counter = 0; epoch_counter < NUM_EPOCHS && epochs_since_new_min < PATIENCE; ++epoch_counter){
        ...
        for (unsigned batch = 0; batch < N_BATCHES; ++batch) {
            ...
            MPI_Allreduce(dl1_local.data(), dl1_aggregate.data(), static_cast<int>(dl1.NTP), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            MPI_Allreduce(dl2_local.data(), dl2_aggregate.data(), static_cast<int>(dl2.NTP), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

            // optimize (compute the adjustments to be applied, according to the selected optimizing algo, e.g. Adam)
            optimizer_ptr->solve(dl1_aggregate, dl2_aggregate);
...
std::pair<NN::type, NN::type> RnnModel::evaluate_accuracy(const std::vector< std::vector<VectorClass> >& x_sequences,
    ...
    return std::make_pair(avg_loss_aggregate, avg_metric_aggregate);
