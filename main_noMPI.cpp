...
//    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    //std::string filename_ = "Data/final_dataset.csv"; // 7feb
    std::string filename_ = "Data/final_dataset.csv";  // 7feb (al posto di riga sopra)
...
int aaaaa = 0;
    ...
    // Network Topology
    const unsigned HIDDEN_NEURONS = 5;                    // neurons in the first hidden layer
    const std::vector<unsigned> WHICH_LAGS = {1,2,24};      // which lags to use as feedback

    // Training and optimization
    const NN::type LEARNING_RATE = 0.0001;                  // learning rate
    const unsigned NUM_EPOCHS = 1;//1000;                       // maximum number of epochs for training
    const unsigned BATCH_SIZE = 64;                         // number of sub-sequences in each mini-batch
    const NN::type LR_DECAY = 0.0;                          // decay of learning rate

    // Regularization. NB: regularization is implemented as WEIGHT DECAY
    ...
    for (auto & elem : GLM_fitted)
        std::cout << elem << std::endl;
    std::cout << "\n" << std::endl;
    for (auto & elem : GLM_predicted)
        std::cout << elem << std::endl;
    std::cout << "\n" << std::endl;
    ...
    if (NN::OUTPUT_NODES == 1){
        ...
        //rnn.set_loss("GaussianWinkler", 1.00);
        rnn.set_metric("MSE_metric"); //("None");
    }
